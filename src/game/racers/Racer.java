...
public abstract class Racer {
    ...
    public Point move(double friction){ //method for racer to show his current location on track
        int flag = 0; // flag indicates if the mishap exists or not
        double newAcc=this.getAcceleration(); // we get the original acceleration of the racer, and we apply our calculation on the temp acceleration
        if(hasMishap()==true){ //we check if there is a mishap
            ...
            if(Fate.breakDown()==true){
                ...
                flag = 1; // a mishap was generated, it maybe fixable and maybe not 
            }
        }

        if (flag == 1) { // if a mishap exists we need to calculate the new acceleration of the racer and reduce the duration of the mishap by 1 turn.
